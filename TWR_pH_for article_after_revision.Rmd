...
lakes%>%full_join(lakesRMÖ)%>%
  mutate(year1=year(Provdatum), 
...
lakes2 %>%
  ...
  mutate(
    ...
    #     As_change=(last(log_as_mg_l)-first(log_as_mg_l))/(last(year)-first(year)),
         ...
         Conductivity_change=(last(log_kond_25_m_s_m)-first(log_kond_25_m_s_m))/(last(year)-first(year)),
...
lakes2a %>%
  ...
  rename(
    ...
    #     As_start=log_as_mg_l_start,
         ...
         Conductivity_start=log_kond_25_m_s_m_start,
         ...
         elevation=elev_utl) ->lakes_numeric

```
#############################################################################
# PCA - Principal components to define a thematical coordinate system and TWR
#############################################################################


Run the PCA
```{r}
pH.pca <- prcomp(lakes_numeric[,c("Ca_start", "Cl_start", "Conductivity_start", "Mg_start", "Na_start",  "Alk_start", "pH_start", "SO4_start", "Ca_change", "Cl_change", "Conductivity_change", "Mg_change", "Na_change",   "SO4_change", "mixed_forest",
         ...
         "open_wetland", "outcrop","temp_mean", "precip_mean", "temp_start", "precip_start", "temp_change", "precip_change", "catchment_area",  "elevation")], center=TRUE,scale=TRUE)  
```

Visualize the PCA

Figure S.1: First and second principal component, all variables
```{r}
fviz_pca_var(pH.pca,
...
Figure 2: First and second principal component, 18 most important variables
```{r}
fviz_pca_var(pH.pca,
             col.var = "contrib", # Color by contributions to the PC
...
Figure S.2: Third and fourth principal component, all variables
```{r}
fviz_pca_var(pH.pca,
             axes=c(3,4),
             col.var = "contrib", # Color by contributions to the PC
...
Table S.2: PCA loadings
```{r}
library(writexl)
res.var <- get_pca_var(pH.pca)
df<-round(as.data.frame(res.var$coord)[,1:4], digits=3)
loadings <- data.frame(Variable = row.names(df), df)
 
 write_xlsx(loadings, path = "pca_result.xlsx")
```
...
Extract the coordinates for individual observations from the PCA and combine with the original lakes data set.
```{r}
...
Figure 3: Thematically weighted regression coefficients on PCA coordinates
```{r}
...
lakes3_pca_res2011%>%arrange(abs(trend))%>%ggplot( aes(x=Dim.1,y=Dim.2))+
  ...
  theme_classic()+
  ylab("PCA component 2 (Change in chemistry)")+
  xlab("PCA component 1 (Initial chemistry levels, elevation)")+
 theme(axis.title = element_text(size=16), legend.key.size = unit(1, 'cm'))
```

Figure S.3: Thematically weighted regression model on the third and fourth PCA components.
```{r}
...
scale_colour_gradientn(colors=c("darkblue", "lightblue3", "darkolivegreen1", "yellow", "orange", "brown2", "darkred"),values=c(0, 40, 45,  50, 55, 60, 100)/100,
  ...
  labs(colour="Linear trend slope")+
  theme_classic()+
  ylab("PCA component 4 (Initial levels of pH and alkalinity)")+
  xlab("PCA component 3 (Overall level of precipitation)")+
     theme(axis.title = element_text(size=16), legend.key.size = unit(1, 'cm'))
```

##############################################################################
# PLS - Partial least squares to define a thematical coordinate system and TWR
##############################################################################

Fit PLS 
```{r}
pls_pH = plsreg1(lakes_numeric[,c("Ca_start", "Cl_start", "Conductivity_start", "Mg_start", "Na_start",  "Alk_start", "pH_start", "SO4_start", "Ca_change", "Cl_change", "Conductivity_change", "Mg_change", "Na_change",  "SO4_change", "mixed_forest",
         "mixed_coniferous", "vegetated_other", "artificial_sufaces", "other", "deciduous_forest",       "semi_urban", "inland_water", "forest_wetland", "pine_forest", "young_forest", "arable_land",
         "open_wetland", "outcrop","temp_mean", "precip_mean", "temp_start", "temp_change", "precip_start", "precip_change", "catchment_area", "elevation")], lakes_numeric[, "pH_change", drop = FALSE], comps = 12)
```

Table S.3. PLS loadings. 
```{r}
pls_pH$x.loads

df_pls<-round(as.data.frame(pls_pH$x.loads)[,1:4], digits=3)
loadings <- data.frame(Variable = row.names(df_pls), df_pls)
 
 write_xlsx(loadings, path = "pls_result.xlsx")
```


Figure S.4: First and second component defined by pls with all variables. 
...
plot_pls<-plot_plsreg1(pls_pH, comps = c(1, 2), where=c("t","t"), main="", 
     ...
     col_ylabel="black")+
  theme(axis.title = element_text(size=12), legend.key.size = unit(0.6, 'cm'))
...
Figure 4: First and second component defined by pls with the 18 most important variables. 

```{r}
source("plot.plsreg1 with ggplot_changed.R")
rbPal <- colorRampPalette(c("#00AFBB", "#E7B800", "#FC4E07"))

pls_pH_selected<-pls_pH

names<-dimnames(pls_pH$R2Xy)[[1]][1:36]
colors<-data.frame(rbPal(50)[as.numeric(cut(max,breaks = 50))])
rownames(colors)<-names
colnames(colors)<-"color"


df<-cbind(pls_pH_selected$R2Xy[c("Ca_start","Cl_start","Conductivity_start" , "Mg_start", "Na_start",         "SO4_start","Ca_change",   "Cl_change", "Conductivity_change", "Mg_change", "Na_change",           "SO4_change"  , "temp_mean",    "temp_start",      "temp_change",         "precip_change", "elevation", "mixed_coniferous"),1],
          pls_pH$R2Xy[c("Ca_start","Cl_start","Conductivity_start" , "Mg_start", "Na_start",          "SO4_start","Ca_change",   "Cl_change", "Conductivity_change", "Mg_change", "Na_change",           "SO4_change"  , "temp_mean",    "temp_start",      "temp_change",         "precip_change", "elevation", "mixed_coniferous"),2])

max_selected <- df[,2] #df is cumulative, extract the R2 for both components together

bb1<-pls_pH$cor.xyt[c("Ca_start","Cl_start","Conductivity_start" , "Mg_start", "Na_start",     "SO4_start","Ca_change",   "Cl_change", "Conductivity_change", "Mg_change", "Na_change",           "SO4_change"  , "temp_mean",    "temp_start",      "temp_change",         "precip_change", "elevation","mixed_coniferous", "pH_change") ,]
pls_pH_selected$cor.xyt <- bb1

colors_selected<-colors[c("Ca_start","Cl_start","Conductivity_start" , "Mg_start", "Na_start",     "SO4_start","Ca_change",   "Cl_change", "Conductivity_change", "Mg_change", "Na_change",           "SO4_change"  , "temp_mean",    "temp_start",      "temp_change",         "precip_change", "elevation","mixed_coniferous"),]

plot_pls<-plot_plsreg1(pls_pH_selected, comps = c(1, 2), where=c("t","t"), main="", 
             length=0.1, angle=30, lwd=0.8, col_xarrows = colors_selected ,
     col_xlabels = colors_selected, col_yarrows = "black",
     col_ylabel="black")+
  theme(axis.title = element_text(size=12), legend.key.size = unit(0.6, 'cm'))
...
```
Figure 5: Thematically weighted regression coefficients on PLS coordinates
```{r}
...
lakes3_pls_res2011%>%arrange(abs(trend))%>%ggplot(aes(x=t1,y=t2))+
 ...
 scale_colour_gradientn(colors=c("darkblue", "lightblue3", "darkolivegreen1", "yellow", "orange", "brown2", "darkred"),values=c(0, 40, 45,  50, 55, 60, 100)/100,
  ...
  labs(colour="Linear trend slope")+
  theme_classic()+
  ylab("PLS component 2 (Initial levels of chemistry, elevation)")+
  xlab("PLS component 1 (Changes in chemistry)")+
   theme(axis.title = element_text(size=16), legend.key.size = unit(1, 'cm'))
```
...
Figure 6 (left): Geographically weighted regression model
```{r}
...
Figure 6 (mid): TWR-PCA in a geographical perspective
```{r}
...
Figure 6 (right): TWR-PLS in a geographical perspective
```{r}
...
```{r}
library(ggpubr)
ggarrange(gwr.point_pH, gwr.point_geographical_pca, gwr.point_geographical, ncol=3, labels="AUTO",hjust=-5, common.legend = TRUE, legend="bottom")

```

Results not shown in article. Analysis without the most outlying observations in the PLS coordinate system. 
